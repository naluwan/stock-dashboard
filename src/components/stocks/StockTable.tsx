  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">
        {formatShares(stock.totalShares, stock.market)}
      </td>
      <td className="px-4 py-3 text-right">
        {privacyMode ? (
          ...
          <>
            ...
            {isUS && (() => {
              const twdCost = stock.purchases.reduce((sum, p) => {
                const rate = p.exchangeRate || usdRate;
                return sum + p.shares * p.price * rate;
              }, 0);
              return twdCost > 0 ? (
                <span className="block text-[10px] text-gray-400">
...
export default function StockTable({ stocks, onEdit, onDelete, usdRate = 0, privacyMode = false }: StockTableProps) {
  ...
  return (
    <div className="overflow-x-auto rounded-xl bg-white shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700 -mx-4 sm:mx-0">
