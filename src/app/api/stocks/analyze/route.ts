...
1. **均線策略**：判斷黃金交叉/死亡交叉、均線多空排列
2. **RSI 策略**：超買(>70)/超賣(<30)訊號
3. **MACD 策略**：趨勢動能、柱狀圖變化方向、是否出現背離
4. **KD 隨機指標**：K/D 交叉方向、超買超賣
5. **布林通道策略**：股價在通道中的位置、是否觸及上/下軌
6. **量價分析**：量增價漲/量縮價跌判斷
7. **海龜交易法**：是否突破 20 日 / 60 日高低點
8. **動能策略**：近期漲跌幅趨勢判斷
9. **支撐壓力**：根據均線和布林通道判斷關鍵價位
...
export async function POST(request: NextRequest) {
  try {
    ...
    const analysis = completion.choices[0]?.message?.content || '分析失敗';
