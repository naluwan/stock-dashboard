...
// 強制 Vercel function 跑在美國東岸，確保 OpenAI API 不被地區封鎖
...
async function callGemini(prompt: string): Promise<string> {
  ...
  for (const modelName of modelNames) {
    try {
      ...
      return result.response.text() || '分析失敗';
    } catch (err) {
      console.error(`Gemini ${modelName} failed:`, err);
    }
  }
  throw new Error('Gemini 所有模型額度已用完，請稍後再試');
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasOpenAI && !hasGemini) {
      return NextResponse.json({ error: '未設定 OPENAI_API_KEY 或 GEMINI_API_KEY' }, { status: 500 });
    }
    ...
    // 3. 呼叫 AI：優先 OpenAI（Vercel 美國機房），失敗則 fallback Gemini
    let analysis: string;
    ...
    if (hasOpenAI) {
      try {
        ...
        console.error('OpenAI failed, trying Gemini fallback:', err);
